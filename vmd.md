...
単眼動画からの推定では、体がカメラに向かって前後に傾く角度がほとんど決まらない（カメラの方へ10度傾けても、画像上では体の縦の長さが1.5%縮むだけ）。左右の傾きは画像にそのまま写るので正しく推定される。床の傾き（ステージ4）は足のかかと・つま先から求めるので、足が床に平らに着いていても、その上の体全体がカメラの前後に傾いたまま残る。体の縦のライン（骨盤→首など）をフレームごとに鉛直に直すと、お辞儀や前傾姿勢などの本当の傾きまで消えるので使わない。代わりに「全身の重心は、数秒の平均では接地している足の上にある」という物理的な条件を使う（重心と足圧中心の差は重心の水平加速度に比例するので、時間平均では速度の変化分しか残らない。お辞儀では腰が後ろへ引かれ、重心は足の上に残る）。

- 全身の重心は、関節の位置と体の部位の質量比・重心の位置（de Leva 1996）から求める。
- 支持点は、接地している足のかかと・つま先のうち、その足の最下点から2cm以内の点（かかとを上げていればつま先だけ）。初期姿勢（直立）での「支持点の中心→重心」の前後のずれを、骨盤の左右の向きに合わせて回したものを基準（差0）とする。
- 補正角 θ ごとの、重心と支持点のカメラの奥行き方向（光軸を水平面に投影した向き）の差を、フレームごとに次の損失にする。
  - 片足接地：その足の支持点の中心との差の2乗。
  - 両足接地：両足の支持点の範囲から外れた量の2乗 ＋ 0.1 ×（両足の中心との差の2乗）。どちらの足に体重を掛けているかは分からないので、範囲の中ならどこでもよい。
  - 接地判定から漏れた0.5秒以下の区間（ジャンプとして残した区間を除く）：前後の支持点の中心を直線でつないだ点との差の2乗。漏れたフレームを除くと、重心が足から最も離れる接地の端が抜けて平均が偏る（合成の歩行で3度前後）。
  - 損失は50cmの差で頭打ちにする（接地判定の誤り等）。歩行では重心が支持脚の前後に30cm以上離れるので、これより小さくしない（15cmにすると合成の歩行で8度ずれた）。
- 損失をσ3秒のガウシアンで時間方向に重み付けして足し（窓の中に支持のあるフレームが無い所のために、シーケンス全体の平均を5%加える）、各フレームで最小になる θ を ±25度・0.25度刻みの格子から探して放物線で補間し、σ0.5秒のガウシアンでならす。格子で探すので、頭打ちで損失が凸でなくても最もよい角度が見つかる。窓を0にするとシーケンス全体で1つの角度にする。
- 補正は、足首から下（足の位置・向き・接地点）は変えず、骨盤から上の大域回転（下半身・背骨・首・頭・腕）をカメラの左右の軸まわりに θ 回し、骨盤を奥行き方向に「骨盤の高さ × tan θ」だけ平行移動する（足元を支点に体を起こす）。骨盤の高さはσ0.5秒のガウシアンでならしたものを使う（歩行の1歩ごとの骨盤の上下まで追うと平行移動が揺れ、接地している足が骨盤に対して前後に動いて見えて接地判定に混ざる）。上下の位置と左右の傾きは変えない。脚はMMDの足ＩＫで付いてくる。
- 1回目の接地判定のあとに1度だけ行い、ステージ2で平滑化する前の姿勢（ステージ6bの足首→骨盤の相対位置に使う）にも同じ補正を掛ける。足首から下は動かさないので接地判定は変わらない。
- 座る・寄りかかる・重い物を持つなど、重心が足の上に無い動きでは `lean.enabled: false` にする。
...
| 前後の傾き | 足の支持点の中心から全身の重心への線の、カメラの奥行き方向の傾きの中央値（度。処理前はステージ6cの補正前の姿勢） | 0に近い |
...
- 支持点の中心から重心への線の前後の傾きの処理前後（フレームごとの点と窓の平均）と、掛けた補正角（ステージ6c）
- 届く高さへのクランプの補正量カーブ
...
- 前後の傾き：高さ1.6m・10度見下ろしのカメラの方へ（から）体全体を10度傾けた合成データ（前進する歩行、その場の足踏み）で、出力の背骨の傾きが傾けていない推定と3度未満、骨盤と足ＩＫの奥行き方向の差が4cm未満になること。傾けていない推定の補正は2度未満であること。お辞儀（上半身を30度前へ倒し、腰を引いて重心を足の上に残す）の前傾を残すこと。画像の左右方向（カメラの前を横切る向き）の傾きは補正しないこと。補正が足首から下を動かさず、重心の式と一致すること。
