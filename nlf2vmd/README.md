...
| 体全体が前後や左右に傾いている | `info.floor` の `tilt_deg`・`tilt_mode`。うまくいかなければ `floor.tilt_method: plane`（従来の方法）や `floor.align_tilt: false` と比べる |
...
両足が浮いたままにならないこと（ジャンプは残ること）、回転の補正（中立姿勢の SMPL は MMD でも中立姿勢になる・
